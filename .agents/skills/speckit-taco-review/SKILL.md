...
6. If any source edits remain conflicted or unavailable, preserve the reviewed Taco unchanged and report what is blocked. Otherwise invoke `$speckit-taco-update "<FEATURE_DIR>"` to refresh and present the same Taco. For Handoff-only input, resolve `FEATURE_DIR` from the confirmed active feature; do not infer it from the title or an untrusted path.
