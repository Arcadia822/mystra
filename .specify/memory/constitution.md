...
Mystra changes must preserve the documented MVP boundary unless the boundary is explicitly amended first. GitHub connection methods are deployment-aware: self-hosted Mystra supports explicit PAT connections behind `SecretProvider`; the platform-operated Mystra GitHub App is hosted-only. The open-source tree may retain hosted App code and tests, but self-hosted entry points must report a stable unavailable capability and fail closed. Hosted OAuth verifies that an authenticated actor may bind an installation to a Team; App installation tokens remain short-lived; PAT plaintext stays behind a protected SecretProvider boundary. Durable relational state may store only non-secret connection metadata, opaque references, and authenticated encryption envelopes whose per-secret DEK is wrapped by a KEK held outside RDB. Every Project binds one exact connection and connection modes never silently fall back. Mystra's MVP is a flexible software factory: standardized requirements enter as Tasks, Providers execute under a program-owned Standard Execution Prompt, optional Agent Context supplies lower-priority behavior guidance, and reviewable results leave the factory. Task owns a Mystra `status` distinct from external Issue status and Session execution state. Feature 051 separates `mystra`, the Control Plane management CLI, from `mystra-agent`, the workload-local execution-context CLI. The latter uses a short-lived per-Session execution code to retrieve minimum sufficient Task/Project/Issue-reference/Workspace context and request allowlisted Task-status transitions without accepting arbitrary Task IDs. In the first self-use path the executing Provider uses the host user's authenticated `linctl` to read Linear and authenticated `gh` to publish a PR; Mystra does not proxy, credential, fall back, query, or verify those operations. Agent-authored PR and self-test notes are unverified. A TaskExecutionContext freezes an optional initial Agent snapshot, associates the first goal/autopilot Session, and authorizes every Task Session through an independent short-lived code without introducing a parallel production state machine. Feature 057 introduces only the fixed, program-owned `mystra.workflow`; dedicated `TaskWorkflowState` may contribute fixed Session guidance, Task-scoped stage transitions and source-aware Skill projection, but Workflow stage never becomes Task, Session or TaskExecutionContext state. Its implementation keeps an internal future replacement seam without establishing generic Harness Resources, attachments, registries or plugin runtime capability. Do not introduce a general-purpose logs API, arbitrary stdout/stderr persistence, retry API, arbitrary callbacks, quality-gate fix loops, webhooks, Issue write-back, a general-purpose Integration management catalog, Claude CLI, Kubernetes sandbox workloads, cross-runner shared caches, arbitrary per-repository secret management, standing orders, arbitrary triggers, automatic Workflow routing, remote Harness plugins, workflow DAG/node execution or configurable Production Recipes as incidental work. A Team-authorized, typed, schema-validated, bounded and redacted TaskExecutionContext/Session event history is permitted for production truth; it does not authorize cross-Task global search, a general activity feed, or a log product. Self-hosted Mystra provides single-node human username/password authentication and Owner/Admin/Member Team RBAC as in-scope capabilities; registration grants each human User an initial Team they own, every User always belongs to at least one Team, and Team is the top-level tenant boundary (feature 043 owns this contract). Self-host identity introduces no email dependency and no long-lived Agent/workload identity. Hosted multi-tenant caller identity federation, caller-login OAuth (SSO/social), managed platform secrets, hosted Team administration, public multi-tenancy, and installation lifecycle handling remain prerequisites owned by explicit hosted phases, not capabilities that self-hosted code may assume already exists. PostgreSQL and user-configured Supabase-backed PostgreSQL are approved RDB deployment targets, but they do not authorize public multi-tenancy, managed database provisioning, or hosted Team administration.

### II. Typed Contracts at Service Boundaries

Control-plane APIs, CLI payloads, Runner protocol payloads, MCP tools and Integration capabilities must use explicit TypeScript and Zod contracts. Team is the tenant boundary: Agent, Task, Project, TaskExecutionContext and Session are distinct Team-scoped objects. Agent and Task do not belong to Project. Feature 057's `TaskWorkflowState` is dedicated Task-scoped workflow state, not a top-level product object, generic Resource or attachment. Task owns `status`: pending, in_progress, blocked, done, or canceled. `blocked` means execution needs handoff to a Human; review, authorization, waiting for an answer, waiting for information and other handoff reasons are deferred structured detail rather than top-level states. Task also owns nullable `runtimeId` as its Runtime Context: first Session launch resolves it from the selected Provider and writes it atomically; after the first non-null write it is immutable, and every later Session must run on that Runtime. A pending Task's first Session launch atomically moves pending to in_progress and creates one TaskExecutionContext in a short RDB transaction; after commit it prepares or reuses the exact `(taskId, runtimeId)` Workspace. Workspace readiness idempotently leads to the TaskExecutionContext's first goal/autopilot Session. The TaskExecutionContext-scoped workload may only report blocked with a note or resume in_progress through a dedicated scoped transition contract; Human owns done and canceled and may resolve blocked to in_progress or done. Every transition is allowlisted, revision-safe, idempotent and append-only audited. Task has no error or failed status; Session, TaskExecutionContext and Workflow failures never automatically mutate Task status. TaskExecutionContext freezes all Agent identity fields or none but owns no parallel production lifecycle. Session belongs to neither Task nor Project as an ownership parent; it remains a Team-scoped execution object and MAY reference its driving TaskExecutionContext plus optional Task/Project context. Session launch resolves Runtime, Provider, optional Agent Context and execution Context, composes the immutable program-owned Standard Execution Prompt with lower-priority program-owned fixed Workflow guidance and optional Agent Context, and atomically persists the Session, frozen effective prompt evidence, ready Task Workspace attachment, and first user message before Runtime/Provider I/O. Dynamic Workflow Stage, Action and Skill facts remain CLI-resolved and never enter the frozen prompt. A Session workload receives Workflow capability only when launch can bind it to one exact active `TaskWorkflowState`; disable invalidates that capability, and an old Session may not resolve a later re-enabled state merely because it references the same Task. A Task may have independent Workspace records on different Runtimes for future synchronization, but the current Session path uses only its locked Runtime; cross-Runtime synchronization, Task Runtime migration and failover are deferred. Mystra defines no Turn business object; message identity is only command idempotency and SessionEvent correlation. Features 048/049/050 provide the underlying Task Workspace, Session launch/continuation and observation contracts; feature 051 adds the thin Task production context above them; feature 052 replaces mandatory Agent assignment without introducing a default/sentinel Agent or a second Workspace type; feature 054 supersedes the original Task-global Workspace cardinality, replaces 051's six-state vocabulary with the five-state handoff contract, and renames that internal record to TaskExecutionContext; feature 057 adds only fixed Workflow state and commands. Workspace preparation claim/lease and Workflow Skill reconcile are materialization fencing, not Session Runtime capacity, slot or execution occupancy.

### III. Providers Are Replaceable Boundaries

Mystra uses Open Agents as a source-authoritative framework baseline but owns its provider and execution boundaries. RDB, Issue, sandbox, repository, and Agent integrations must sit behind explicit Mystra-owned contracts. SQLite, PostgreSQL, and Supabase-backed PostgreSQL are selectable RDB deployments behind the same `RdbProvider`; Supabase is a PostgreSQL deployment profile rather than a separate domain contract. GitHub supplies the active remote RepoProvider and repository-scoped IssueProvider; Linear supplies a read-only IssueProvider. Runtime is a first-class, replaceable execution backend that advertises which Provider capabilities (agent CLI / protocol families) it can run, expressed source-agnostically so a host-discovered Runtime and a future image-declared Runtime share one contract. A host-bound Runtime enrolled by the TypeScript `mystra-runner` — covering registration, Provider discovery plus availability confirmation, and heartbeat/status — is in MVP scope; single-machine Docker is one such sandbox provider rather than the sole execution model, and host worktree direct execution is the intended default execution direction. Feature 047 owns Task context and exact Issue intake, feature 048 owns Task Workspace preparation, feature 049 owns Task-bound Session launch/continuation and typed event history, feature 051 introduced Task status, `mystra-agent` context/status surfaces, one Task execution context and exactly one goal/autopilot Session, feature 052 owns the Standard Execution Prompt plus optional Agent Context contract, feature 054 owns the current five-state Task vocabulary, handoff semantics, and `TaskExecutionContext` name, and feature 057 owns the single fixed `mystra.workflow`. Its definition, prompt contribution, Task-scoped state commands and Skill projection are grouped behind an internal module boundary that a future explicit specification may replace with a Harness plugin contract. This is not permission to add a general WorkflowProvider, Resource model, handler registry, remote plugin runtime, workflow node/DAG executor, arbitrary trigger system, automatic router, marketplace or configurable Production Recipe engine. Attempt-owned heartbeat/event subscriptions, multiple Session coordination, generic Artifact/Delivery contracts, platform-proxied external CLIs, PR/self-test verification and Production Recipes require explicit follow-up specifications.

### IV. Runner Isolation and Secret Hygiene

Runner hosts connect outbound to the control plane; the control plane must not require inbound access to a runner host. A host-bound Runtime advertises the Provider CLIs it has discovered and confirmed available without baking their credentials or login state into the platform. Feature 051 additionally assumes host-local `linctl` and `gh` are installed and authenticated for the same OS user as the executing Provider; their credentials remain owned by those tools and are never returned by `mystra-agent`. The `MYSTRA_EXECUTION_CODE` is a separate, short-lived, revocable per-Session capability scoped to the TaskExecutionContext and must not appear in prompts, ordinary logs, status notes, or plaintext persistence. Where a provider uses containers, runner daemons may use the host Docker socket while task containers must not mount it; this Docker-socket allowance is provider-specific, not a universal runner assumption. Secrets are injected at runtime through environment variables or read-only files and must not be committed or baked into images.
...
UI-facing specs use an independent Spec Prototype app for interaction review,
but that app and production Web MUST consume the same Mystra theme stylesheet,
UI component implementations, icon set, and shell layout contracts. A
prototype MUST NOT copy production component markup, SVG, tokens, or CSS anatomy
to simulate reuse. If a required primitive is missing, add it to the shared UI
package first and verify both consumers. Only mock data, feature composition,
and explicitly feature-local experimental layout may remain prototype-owned.
...
- `mystra` is the Control Plane management CLI. `mystra-agent` is a separate
  workload-local adapter whose short-lived per-Session execution code addresses only the
  current TaskExecutionContext; it MUST NOT accept arbitrary Task IDs or expose external
  credentials.
- Web UI is a secondary client. Its demo shell exposes New, Search, Inbox, and
  ...
  automation catalog. Task assignment and status remain Task surfaces.
...
- 2026-08-24: Feature 057 introduces only the fixed, program-owned
  ...
  transitions and source-aware Skill projection while remaining separate from
  `TaskExecutionContext`, `Task.status` and `Session.state`. The implementation
  keeps an internal future replacement seam, but generic Harness Resources,
  attachments, registries, editable definitions, remote plugins, automatic
  routing, arbitrary triggers, workflow DAG/node execution, a marketplace and
  configurable Production Recipes remain excluded pending a follow-up amendment.

- 2026-08-17: Feature 054 renamed the internal `Harness` contract to
  `TaskExecutionContext` and the Task lifecycle field from `productionStatus`
  to `status`. Task also gained a top-level JSON-object `metadata` field. Under
  the pre-0.1 policy these are direct replacements across shared contracts,
  persistence, APIs, CLI, UI, fixtures, and tests: no aliases, dual reads,
  shadow columns, `TaskLabel` relation, ordinal, or normalized key/value fields.
  TaskExecutionContext remains an internal coordination record with no product
  navigation or independent management surface.

- 2026-08-13: Established `apps/spec-prototype` as the reusable, independent
  UI-facing Spec-Kit prototype app and `packages/ui` as the shared production
  and prototype implementation boundary. Prototype review now requires
  code-level reuse of theme, primitives, icon set, and shell layout contracts;
  copied static HTML/DOM/SVG/CSS is not acceptable evidence of portability.

- 2026-08-13: Feature 054 replaced the six-state Task vocabulary introduced by
  051 with `pending`, `in_progress`, `blocked`, `done`, and `canceled`.
  `waiting_for_review` is removed and its handoff semantics are merged into
  `blocked`, displayed as Needs handoff / 待接手. Review, authorization, waiting
  for an answer, waiting for information, and other causes are deferred handoff
  reasons, not status values. Task continues to have no error/failed state;
  Session/TaskExecutionContext failures remain independent facts. Under the pre-0.1
  policy, shared schemas, persistence fixtures, transition services, CLI,
  prompts, APIs, UI, tests, 053 Overview, and current docs must be replaced
  directly without aliases, shims, or dual reads.
...
- 2026-08-11: Named the feature 051 workload client `mystra-agent` and reserved
  `mystra` for Control Plane management. Added Task-scoped execution-code
...
- 2026-08-11: Amended feature 051 to add the thin Task production state machine
  ...
  TaskExecutionContext owns no parallel lifecycle, and only Human actors may complete or
...
- 2026-08-11: Reframed Mystra's product direction as a flexible software factory. Task is the
  production order, Agent is the responsible producer, TaskExecutionContext identifies one
...
- 2026-08-17: Feature 054 superseded the Task-global Workspace cardinality from
  048. Task Workspace is unique by `(taskId, runtimeId)`; Session launch uses
  Provider availability for the first Runtime resolution, atomically locks nullable
  `Task.runtimeId`, and owns automatic setup/retry/reuse. The Runtime Context is
  immutable after first write and all later Sessions stay on that Runtime.
  Human operators do not manage Workspace readiness. Cross-Runtime Workspace synchronization is
  explicitly deferred, as are Runtime migration/failover, while all Workspaces retain the same attachment type.

- 2026-08-10: Features 048/049/050 were narrowed to Task-bound Session delivery.
  Feature 048 originally owned one Runtime-affine Task Workspace and a strict ready
  `task/shared-mutable` attachment resolver；its Task-global cardinality is superseded
  by the 2026-08-17 feature 054 amendment. It does not create Session、initial
  ...
  Workspace type remains singular, so future preparation for the deferred
