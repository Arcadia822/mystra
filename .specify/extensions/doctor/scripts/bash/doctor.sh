...
AGENT_FOUND=false
while IFS='|' read -r path agent_name; do
    [ -n "$path" ] || continue
...
if [ -d "$PROJECT_ROOT/.specify/scripts/bash" ]; then
    for name in $EXPECTED_SCRIPTS; do
        script="$PROJECT_ROOT/.specify/scripts/bash/${name}.sh"
        if [ -f "$script" ]; then
            if [ ! -x "$script" ]; then
...
if [ -d "$PROJECT_ROOT/.specify/scripts/powershell" ]; then
    for name in $EXPECTED_SCRIPTS; do
        script="$PROJECT_ROOT/.specify/scripts/powershell/${name}.ps1"
        if [ -f "$script" ]; then
    ...
    echo "  [NOTE] .specify/scripts/powershell/ not found"
fi
...
if [ -d "$PROJECT_ROOT/.specify/extensions" ] && [ "$(find "$PROJECT_ROOT/.specify/extensions" -mindepth 1 -maxdepth 1 -type d 2>/dev/null)" ]; then
    echo "  [OK] Extension directories found"
else
