...
describe("executeSessionAssignment", () => {
  ...
  it("keeps the Provider session identity the adapter supplies for AgentOS continue", async () => {
    const appendEvents = vi.fn(async (_assignment: SessionClaimAssignment, _events: SessionEventInput[]) => undefined);
    const assignment = {
      session: {
        id: "00000000-0000-4000-8000-000000000001", teamId: "00000000-0000-4000-8000-000000000002",
    ...
    const runProcess = vi.fn(async (_command: ProviderSessionCommand) => ({ exitCode: 0, stdout: "", stderr: "" }));
    await executeSessionAssignment({
      assignment,
      client: { appendEvents },
      workspace: { resolveReadyWorkspace: vi.fn(async () => ({ directory: "/workspace" })) },
    ...
    expect(runProcess.mock.calls[0]![0].environment).toMatchObject({
      MYSTRA_SESSION_ID: "00000000-0000-4000-8000-0000000000aa",
      MYSTRA_SESSION_MODE: "continue",
    });
  });
