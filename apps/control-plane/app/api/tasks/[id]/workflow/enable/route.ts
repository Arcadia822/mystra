...
import { workflowEnableResponseSchema } from "@mystra/shared";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const subject = await requireHumanSession(db, request, "task-workflow-enable");
    const active = await requireTeamPermission(db, subject, "team.resource.access");
    const { id: taskId } = await context.params;
    const response = await createWorkflowManagementService(db).enable({
      teamId: active.team.id,
      taskId,
      actor: { userId: subject.user.id, role: active.role },
      request: await request.json(),
    });
    return NextResponse.json(workflowEnableResponseSchema.parse(response));
  } catch (error) {
