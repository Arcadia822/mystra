...
CREATE TABLE "tasks" (
    ...
    CONSTRAINT "tasks_title_length_check" CHECK (char_length(btrim("title")) BETWEEN 1 AND 500),
    CONSTRAINT "tasks_description_length_check" CHECK ("description" IS NULL OR char_length("description") <= 100000),
...
ALTER TABLE "tasks" ADD CONSTRAINT "tasks_runtime_id_fkey" FOREIGN KEY ("runtime_id") REFERENCES "runtimes"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE "tasks" ADD CONSTRAINT "tasks_issue_connection_id_fkey" FOREIGN KEY ("issue_connection_id") REFERENCES "integration_connections"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
