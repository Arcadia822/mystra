...
const defaults = {
  ...
  repo: process.env.MYSTRA_DEFAULT_REPO ?? "",
  source: "api",
...
function usage() {
  console.error(`Usage:
scripts/castrel-job.mjs --task-id CAST-123 --branch mystra/CAST-123-name --title "MR title" --body "MR body" --prompt-file /tmp/prompt.md [--agent codex|copilot] [--no-wait]`);
  process.exit(2);
}
...
for (const required of ["task-id", "branch", "title", "body", "prompt-file", "repo"]) {
  if (!args[required]) {
    usage();
  }
}
...
const payload = {
  ...
  baseBranch: args["base-branch"] ?? defaults.baseBranch,
