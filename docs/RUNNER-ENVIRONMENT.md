...
pull-request creation receive a short-lived credential for the exact connection
bound by the Project：PAT under stock self-hosted，or an installation token in a
future Hosted distribution。`MYSTRA_GITHUB_TOKEN` is not a supported Runner
input.
