...
`mystra` needs to execute coding-agent tasks on private bare-metal infrastructure without exposing the runner host to the public internet. The supplied plan used `10.106.2.127` as the concrete host, but the implementation should treat the runner as a configurable single-host target.
...
Use a control plane backed by the configured `RdbProvider` as the source of truth and private runner daemons that register, heartbeat, pull tasks, append structured events, and publish results over outbound connections.
...
- Workflow providers can be replaced later without rewriting runner protocol or task state.
...
1. Create a task through the control plane.
2. Register a fake runner.
3. Claim the task through outbound runner polling.
