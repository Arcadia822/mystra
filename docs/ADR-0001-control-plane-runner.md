...
Stripe Minions-style practice suggests deterministic workflow nodes around agentic coding loops: clone, test, build, branch push, merge request creation, and final status should be controlled by code rather than left entirely to prompts. The MVP includes a single deterministic `test -> build` gate before push/MR creation, but defers automatic fix loops after gate failure.
...
The MVP intentionally does not provide control-plane caller auth, logs API, retry API, callback URL, or quality-gate fix loops. It does include a single deterministic `test -> build` quality gate before branch push and MR creation. Branch naming, MR title, and MR body remain task/repository responsibilities.
...
- The configured runner host can change without modifying MCP/API or GitLab integrations.
...
- The task container receives GitLab authority in MVP, which is a deliberate trusted-infrastructure tradeoff.
