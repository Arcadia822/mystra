...
3. **Given** two Projects use different runtime images, **When** tasks are submitted for each, **Then** Mystra preserves the per-Project runtime distinction without requiring runner-global image configuration.

---

### Technical Scenario 2 - Resolve Runtime From Project Default And Constrained Task Overrides (Priority: P1)

Mystra resolves an effective runtime contract for each run from the Project default runtime plus permitted task-level overrides. The first version supports one default runtime per Project, while the contract leaves room for future named runtime profiles such as `frontend-dev`, `backend-dev`, `docs`, or `test`.
...
**Independent Test**: Submit one task using the Project runtime and another with an allowed image/context override; verify each claim receives a resolved runtime contract and the Project itself remains unchanged.

**Acceptance Scenarios**:

1. **Given** a Project has runtime config, **When** a caller submits a task without runtime fields, **Then** Mystra resolves the run from the Project runtime config.
2. **Given** a caller provides an allowed task runtime override, **When** the task is accepted, **Then** Mystra records the resolved runtime snapshot for that run without mutating the Project.
3. **Given** a future caller selects a named runtime profile, **When** profile support is implemented, **Then** Mystra resolves from that Project-managed profile before applying allowed task overrides.
4. **Given** a caller requests an unsupported provider, forbidden mount, disallowed context bundle, embedded secret value, or disallowed override field, **When** the task is submitted, **Then** Mystra rejects the task or fails resolution before agent execution begins.

---

### Technical Scenario 3 - Design API/MCP Runtime Boundaries (Priority: P0)

Mystra exposes a clear API and MCP contract for Project runtime config, future runtime profiles, constrained task overrides, context bundle references, and resolved runtime claims.
...
**Independent Test**: Submit Project and task payloads through HTTP and MCP route tests, verifying that accepted payloads match the shared schemas and invalid top-level image fields, unsafe overrides, malformed runtime fields, or unsupported provider fields are rejected at the boundary.
...
3. **Given** a task submits a runtime override, **When** the Project policy permits that override field, **Then** the boundary accepts the payload and runtime resolution records the final snapshot.
4. **Given** a task submits a runtime override for mounts, secrets, or another MVP-forbidden field, **When** it reaches the API or MCP boundary, **Then** Mystra rejects it with a clear validation or policy error.

---

### Technical Scenario 4 - Provide Context Through Explicit Bundles (Priority: P1)

Mystra provides task containers with explicit context bundles, such as agent skills, issue summaries, repository instructions, or operator-provided task context, without baking concrete context contents into the Mystra source tree.
...
**Independent Test**: Configure Project runtime with named context bundles, submit a task, and inspect the claim to confirm the runner receives resolved bundle contracts and access policy rather than hard-coded source-tree paths or prompt fragments.

**Acceptance Scenarios**:

1. **Given** Project runtime includes the `agent-skills` context bundle, **When** a run is claimed, **Then** the runner receives a resolved context-bundle contract that identifies what must be made available and under which access mode.
2. **Given** a task contains task-specific issue context, **When** Mystra prepares the run, **Then** the context is attached as a job-scoped bundle rather than embedded in runner-daemon source text.
...
A platform engineer can add or replace a sandbox provider without changing Project or task submission contracts.
...
**Independent Test**: Define a non-Docker provider shape in contracts and verify the same Project/task runtime config envelope can resolve into provider-specific execution details without adding another Project-level image field.
...
3. **Given** a future provider does not use container images, **When** it resolves runtime config, **Then** Project and task contracts remain unchanged.
...
**Why this priority**: A baseline image can help bootstrap local testing, but concrete runtime configuration belongs to Project/task runtime config and release artifacts, not hard-coded runner-daemon prompt text or source-owned bundled context.

**Independent Test**: Build/use the local baseline image by referencing it from Project runtime config and verify task execution uses the resolved runtime contract rather than global env vars or source-owned context assumptions.
...
2. **Given** a task is submitted for a Project with runtime image config, **When** the run is claimed, **Then** execution uses the resolved runtime contract.
...
- Task override tries to replace runtime image when Project policy does not allow overrides.
- Task override selects a named runtime profile before profile management exists.
- Task override tries to change mounts, secrets, cache, or ports before those override surfaces are explicitly allowed.
...
- **FR-003**: Task submission MUST resolve an effective runtime from Project runtime config unless the caller provides a permitted task-level override.
- **FR-004**: Mystra MUST reject runtime config or task overrides that reference unsupported providers, invalid images, forbidden mounts, embedded secret values, or disallowed context bundles.
...
- **FR-016**: Documentation MUST state the ownership boundary between Project runtime config, task runtime overrides, context bundles, sandbox provider translation, and baseline local templates.
...
- **FR-018**: Task runtime override MUST be allowed only for explicit MVP fields, initially provider/image/context bundle references and metadata as permitted by Project policy; mounts, secrets, cache, and ports remain Project-managed until their management model is designed.
...
- **FR-021**: API and MCP boundaries MUST validate Project runtime config and task runtime override shapes explicitly instead of relying only on downstream persistence parsing.
...
- **Task Runtime Override**: Optional task-level runtime changes permitted by Project policy and captured as part of the run's resolved runtime snapshot. MVP override scope is intentionally narrow.
- **Context Bundle**: Named package of context made available to a run, such as agent skills, issue summaries, repository instructions, operator guidance, or task-specific context. It defines source, access mode, freshness expectations, and failure behavior.
- **Runtime Resolver**: Control-plane logic that combines Project runtime config, task overrides, context bundles, runner compatibility, and policy checks into a resolved runtime contract.
- **Resolved Runtime Contract**: The run-specific, provider-ready contract returned to a compatible runner after Project defaults, task overrides, context bundles, and policy checks are resolved.
...
- Task-level runtime overrides are opt-in and constrained by Project policy.
...
- **SC-001**: 100% of accepted tasks resolve a runtime contract before runner claim.
...
- **SC-007**: The feature documentation enables a future agent to identify the owner of Project runtime config, task overrides, context bundles, sandbox provider translation, and local-only Castrel image context without relying on chat history.
