...
**Independent Test**: Configure Project runtime with named context bundles, submit a job, and inspect the claim to confirm the runner receives resolved bundle contracts and access policy rather than hard-coded source-tree paths or prompt fragments.
...
2. **Given** a job contains task-specific issue context, **When** Mystra prepares the run, **Then** the context is attached as a job-scoped bundle rather than embedded in runner-daemon source text.
