...
- [x] T069 [US3] 先在 `theme-system.test.ts` 写失败测试，锁定 Everforest `codex-theme-v1:{JSON}` exact round-trip、schema version / `codeThemeId` 分工、unknown version 与 synthetic `id` rejection、built-in round-trip 和 canonical dataset。
- [x] T070 [US3] 在 `theme-system.ts` 建立严格 `CodexThemeV1Payload` parser/serializer 与 Mystra metadata adapter，删除 `ControlPlaneThemeDefinition.id`，以 `(variant, codeThemeId)` 统一 registry、Appearance lookup、Settings option 和 DOM dataset。
- [x] T071 [US3] 为 `notion-light`、`linen-light`、`notion-dark` 添加受限 localStorage migration，同时让 runtime 与 hydration bootstrap 输出相同 `codeThemeId`。
- [x] T072 重新渲染 025 Spec View，并运行 147/147 control-plane tests、typecheck、production build、`git diff --check`、Mystra UX skill validation、GitNexus `detect-changes` 与真实浏览器 Appearance/首帧回归。Everforest/Codex v1 exact round-trip、显式字体、light `notion` canonical dataset、刷新恢复、dark `graphite-signal` 恢复和 0 browser console error/warning 均通过。GitNexus 对整个 29-file dirty worktree 报告 71 symbols / 26 flows / CRITICAL，包含 AGENTS/CLAUDE 与并发 spec/skill 改动，不能归因于本主题切片；本切片预编辑 upstream 已单独记录。浏览器数据 API 因现有 `better-sqlite3` native binary 的 NODE_MODULE_VERSION 137/147 不匹配返回 500，但主题 Settings、DOM dataset 与首帧 bootstrap 验证不依赖这些 API，未越界重建用户依赖。
- [x] T073 [US3] 从本机签名 Codex 26.730.61639 / build 6234 的 `app.asar` registry 与 theme modules 提取并核对 28 个 family / 43 个 supported variant，形成带 source version 的静态 `codex-theme-catalog.ts`。
- [x] T074 [US3] 先写失败测试锁定完整 family/count、已知 Codex/Everforest payload、Mystra 双变体/default、官方 variant resolution 和 Graphite/Linen legacy migration；确认 4 项预期失败后再实现。
- [x] T075 [US3] 将原 Graphite dark canonical identity 改为 `mystra`，新增配套 Mystra light explicit tokens，并让 light/dark Appearance 默认均使用 Mystra；所有 Codex catalog entry 继续通过同一 v1 adapter。
- [x] T076 [US3] 将 `graphite-signal` 迁移到 `dark:mystra`、`linen-light` 迁移到 `light:notion`，并让 Mystra 在两个 variant 的共享 dropdown 中置顶、其余 Codex themes 按 label 排序。
- [x] T077 刷新 025 Spec View；通过 27 files / 148 tests、typecheck、production build、skill validation 与 `git diff --check`；GitNexus 对 43-file mixed dirty worktree 报 critical，未错误归因给主题切片；真实浏览器确认 17 个 light / 28 个 dark 选项、Mystra 双变体 CSS 首帧值、旧偏好恢复和 console 0 warning/error。
- [x] T078 [US3] 先补失败测试定义 UI / Content / Code 内部字体角色、Codex UI → Content adapter、单 primary family、Mystra Arial/Georgia/Courier New 与旧 `chatFont`/Graphite stack 迁移；确认 9 项预期失败后实现。
- [x] T079 [US3] 将 Appearance、CSS semantic variables、bootstrap 与双语文案从 Chat 重命名为 Content；保留 `--font-chat` / `--font-size-chat` 仅作 CSS alias，不再作为 normalized preference 字段。
- [x] T080 通过 27 files / 149 tests、typecheck、production build、skill validation 与 `git diff --check`；GitNexus 对 57-file mixed dirty worktree 报 critical，未归因给字体切片；真实浏览器确认旧 Graphite stack 首帧迁移为 Arial/Georgia/Courier New、UI/Content/Code computed stacks、中文 Content 设置文案、code preview 与 console 0 warning/error；在当前仓库 HTTP 服务刷新 025 Spec View 并确认 FR-039。
- [x] T081 [US4] 先扩展 `component-migration.test.ts` 写失败契约，锁定 Sidebar/Main/Right Panel 三列均具有 Header/Content、Right Panel 缺省不占宽、Task 详情显式 opt-in 与 `<=700px` 堆叠；确认新增契约先失败后再实现。
- [x] T082 [US4] 新增页面级 `ShellRightPanel` 注册 seam，并重构 `AppShell` 为 `SidebarHeader + SidebarContent`、`MainHeader + MainContent`、可选 `RightPanelHeader + RightPanelContent`；注册页面卸载时清理，不新增 pathname content switch。
- [x] T083 [US4] 将 Task 详情的 `Create Session` 表单迁入全局 Right Panel，保持表单状态、错误提示和 canonical Session API 不变；其他页面不启用 Right Panel。
- [x] T084 通过 focused contract test、27 files / 150 tests、typecheck、production build 与 `git diff --check`；真实浏览器在 1440/1024/320px 覆盖桌面三列、sidebar overlay 布局、`<=700px` Main 后堆叠、无 panel 页面自动扩展、route cleanup、可访问语义与 0 console error/warning。1024 与 320 两侧实测覆盖 768 所属的同一 `701–1024px` CSS band；刷新 025 Spec View 并运行 GitNexus mixed-worktree detect-changes。
