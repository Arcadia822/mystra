...
**Schema Supersession Notice (2026-08-06)**: This Spec is historical evidence for introducing `Project`, SQLite, and the `RdbProvider` boundary; it is **not** the current entity or schema contract. Its `Job`, `Run`, `RunnerSession`, `RunEvent`, and database `Artifact` entities, including FR-012 and related acceptance criteria, were superseded by `038-task-session-model` and subsequent product decisions. The pending `040-prisma-rdb` ER now excludes Session persistence entirely, in addition to Project `defaultAgent`, runtime/image and prewarm ownership, ContextBundle persistence, Runner persistence, and the unprefixed `baseBranch` schema; only `repositoryBaseBranch` remains in the first Prisma Project model. `sessions`, `session_events` and `artifacts` are excluded. Future Session, Task Activity, Artifact, Context delivery, Runtime and Runner designs require separate specifications. Do not derive a migration or Prisma schema from this document.
