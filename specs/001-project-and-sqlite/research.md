...
## Decision: Project Is The Stable Job Configuration Owner
...
- Keep repo/baseBranch/agent/image on every job: rejected because it repeats configuration and makes callers responsible for platform defaults.
- Keep a config file seed only: rejected because operators need API/MCP-driven Project CRUD.

## Decision: Jobs Store Resolved Snapshots

**Rationale**: Historical jobs must remain explainable after Project edits. Store `project_id` for traceability and also store resolved repo/baseBranch/agent fields on `jobs`.

**Alternatives considered**:

- Query live Project for every job display: rejected because Project edits would rewrite history.
- Store only a JSON spec: rejected because it weakens relational queries and duplicates the current local-store shape.

## Decision: No Transaction Wrapper Around Project Lookup + Job Insert For MVP

**Rationale**: Job stores a snapshot, not a live reference. Current single-machine SQLite path has modest concurrency, and Project mutation after lookup does not corrupt the created job.
...
## Decision: Project.image Is Runtime Truth

**Rationale**: Different GitLab/GitHub projects may need different runtime images. A runner-global `MYSTRA_RUNNER_IMAGE` prevents per-project isolation.
