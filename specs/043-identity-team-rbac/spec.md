# 功能规格：开源本地用户、Personal Team 与 RBAC
...
**Goal**: 开源 Mystra 部署后直接进入本地登录页；Human User 使用 username/password 登录和注册，每个 User 始终拥有一个可改名、可加成员的 Personal Team，并可切换、创建和删除其他 Team；Account Settings 支持修改 password 与 display name，Team Settings 支持成员增删和角色管理。
**Input**: 外部安装流程未来负责提供默认 `admin/admin` 与该用户名为 `Personal` 的 Personal Team；043 不实现安装器或 seed 命令，只定义并消费部署完成后的 bootstrap contract。self-host 不引入 email。SaaS/SSO、Agent authentication、Agent key、Sandbox workload identity 与强认证因子本期不做。
...
**Independent Test**：使用安装流程预置的 `admin/admin` 登录，验证首次登录强制修改 password；注册第二个 User，验证其 Personal Team 自动创建；修改 display name 和 password，验证旧 password 与被撤销 session 不再可用。

**Acceptance Scenarios**：

1. **Given** 部署已按 bootstrap contract 预置 `admin/admin` 与 `Personal` Team，**When** 浏览器首次打开 Mystra，**Then** 默认进入登录页，不自动进入应用 shell。
2. **Given** `admin` 使用默认 password `admin` 登录，**When** credential 验证成功，**Then** 系统只允许进入强制修改 password 流程，完成后才允许访问 Team 资源。
3. **Given** 初始化已完成，**When** 新用户提交唯一 username 和有效 password 注册，**Then** 系统原子创建 User、其 Personal Team、Owner membership 与 session。
...
7. **Given** 规范化 username 已被占用，**When** 串行或并发重复注册发生，**Then** 最多创建一个 User/Personal Team，其他请求返回稳定冲突，不产生孤儿 Team 或 membership。
8. **Given** password 错误、User 停用或 session 已撤销，**When** 请求受保护资源，**Then** 系统在业务副作用前 fail closed，并避免泄漏账户是否存在。

---

### User Story 2 - 使用和管理 Personal Team 与其他 Team（Priority: P1）

作为已登录 User，我希望始终拥有自己的 Personal Team，并能创建、切换、重命名和删除其他 Team，以便将个人工作与不同协作范围分开管理。

**Why this priority**：Team 是 Mystra 的租户边界；如果用户无法明确切换 Team，系统将非常高效地把正确数据写进错误租户。

**Independent Test**：注册 User 并验证 Personal Team；创建两个 Team，在 shell 中切换并刷新页面；重命名 Personal Team；删除一个非 Personal Team；验证 Personal Team 和仅剩的最后 Team 均不可删除。

**Acceptance Scenarios**：

1. **Given** 任意 Human User 创建成功，**When** 读取其 Team 列表，**Then** 系统恰好存在一个由该 User 永久拥有、初始 display name 为 `Personal` 的 Personal Team。
2. **Given** User 位于任意 Team，**When** 打开 Team switcher，**Then** 列表只显示其 active memberships，并清楚标识当前 Team 与其 Personal Team。
3. **Given** User 选择另一个 Team，**When** 切换完成或刷新页面，**Then** 后续导航和请求使用新 Team context；无权或已删除 Team 不得恢复为 active context。
4. **Given** User 创建新 Team，**When** 提交有效名称，**Then** 系统创建普通 Team，并将创建者设为 Owner 后切换或提供明确进入动作。
5. **Given** Owner 修改普通 Team 或自己的 Personal Team 的 display name，**When** 保存成功，**Then** switcher、Settings 和成员页面显示新名称；Personal Team 的内部类型与 personal owner 不变。
6. **Given** Owner 管理一个非 Personal Team，且自己还属于至少一个其他 active Team，**When** 确认删除，**Then** Team 被归档并从所有成员的 switcher 中消失，历史业务归属保持可追溯。
7. **Given** Team 是 Personal Team，或它是当前 User 唯一的 active Team，**When** User 查看或调用删除动作，**Then** 删除操作不可用并返回稳定不可删除原因。
8. **Given** 当前 Team 被其他 Owner 删除或当前 membership 被移除，**When** User 发起下一请求，**Then** 系统切换到其 Personal Team 或要求选择其他有效 Team，不继续使用失效 Team context。
...
**Independent Test**：在普通 Team 和 Personal Team 中分别添加已有 User、修改角色、移除成员；验证 Owner/Admin/Member 权限矩阵、最后 Owner与 Personal owner 保护，并确认 API、MCP、CLI 与 Web 的授权结果一致。
...
7. **Given** Team 是某 User 的 Personal Team，**When** 尝试移除或降级 personal owner，**Then** 系统永久拒绝；其他成员仍可正常增删和变更角色。
8. **Given** membership 被移除或停用，**When** User 再次请求 Team 资源，**Then** 新请求立即失败，Team 从其 switcher 中移除。
...
- 每个 User 必须恰好拥有一个 Personal Team；注册事务失败时不得只留下 User 或只留下 Team。
- Personal Team 可以重命名、添加成员和设置角色，但不能删除、转移 personal ownership 或移除 personal owner。
- 普通 Team 的显示名称可以重复；内部 Team ID 是租户边界，不得用名称解析授权。
- Team switcher 只列出 active memberships；被移除、停用或归档 Team 不能由缓存恢复。
- Team 删除是可审计归档而非数据库级联硬删除；历史 Project、Task 和 Session 仍保留 Team ID。
- 未加入任何协作 Team 的 User 仍可通过自己的 Personal Team 使用系统，因此不存在零 Team 的有效 User。
- Role 变更和 membership 删除必须使新请求立即失效；长时操作在外部副作用前重新确认权限。
- password、session token 和 bootstrap secret 不得进入 URL、日志、公共响应或验收证据。
- 数据库暂时不可用时，不得创建半完成 User、Personal Team、membership、role binding 或 session。
...
- **FR-002**：部署完成后的外部 bootstrap contract MUST 提供 username `admin`、初始 password `admin`、对应 Human User 与其初始名为 `Personal` 的 Personal Team；043 MUST NOT 实现 installer 或 seed orchestration。
...
- **FR-009**：本地注册 MUST 只收集 username 与 password，并在同一事务中创建 User、Personal Team、Owner membership 与 session。
...
#### Personal Team, Team lifecycle and active context

- **FR-017**：每个 Human User MUST 恰好拥有一个 Personal Team；该关系使用稳定 personal owner ID 表达，不依赖 Team display name 是否仍为 `Personal`。
- **FR-018**：新 User 注册事务 MUST 原子创建其 Personal Team，并将 User 设为不可移除、不可降级的 personal Owner。
- **FR-019**：Team MUST 是 Mystra 顶层租户；`workspace` 不得被用作 tenancy 同义词。
- **FR-020**：系统 MUST 提供 Team switcher，列出当前 User 的 active TeamMembership，标识当前 Team 和 Personal Team，并允许切换 active Team context。
- **FR-021**：active Team context MUST 持久保存并由服务端验证；无权、已停用或已归档 Team MUST fail closed，并回退到 Personal Team 或明确选择流程。
- **FR-022**：任意 User MUST 能创建普通 Team；创建者 MUST 原子成为该 Team Owner。
- **FR-023**：Owner MUST 能修改普通 Team 和自己的 Personal Team 的 display name；Team display name MAY 重复且不得参与授权解析。
- **FR-024**：Owner MUST 能删除普通 Team；用户可见“删除” MUST 实现为可审计归档并从 active switcher 移除，不得级联硬删除历史业务记录。
- **FR-025**：Personal Team MUST 永远不可删除；任何 Team 若是当前 User 唯一 active Team，其删除动作也 MUST 不可用。
...
- **FR-031**：Owner MUST 能移除或停用普通成员；Admin MAY 管理 Member，但 MUST NOT 修改 Owner、授予 Owner、移除 Owner 或管理 personal owner。
...
- **FR-036**：系统 MUST 保护每个普通 Team 至少一名有效 Owner；最后 Owner 不得被移除、停用、降级或退出。
- **FR-037**：Personal Team 的 personal owner MUST 永久保持 active Owner；其他成员 MAY 被授予 Owner，但不得借此移除、降级或替换 personal owner。
...
- **FR-040**：Team Members 页面 MUST 提供 loading、empty、error、read-only/forbidden、add conflict、last-owner conflict 与 personal-owner-protected 状态。
...
- **FR-042**：Team、Personal Team ownership、Membership、Role、Permission 与 RoleBinding MUST 是 Mystra-owned 领域模型；Better Auth Organization/Team 不得成为产品真相来源。
...
- **FR-045**：SQLite 与 PostgreSQL Prisma schema MUST 覆盖相同 Auth、Personal Team 与 RBAC 逻辑模型，并通过自动 parity 检查阻止无意漂移。
...
- **Team**：Mystra 顶层租户；包含稳定 ID、可重复 display name、kind（Personal 或 Standard）、状态和时间信息。
- **PersonalTeamOwnership**：一个 User 与其唯一 Personal Team 的不可转移所有权事实；不依赖 Team display name。
...
- 部署完成后进入登录页，并消费外部 installer 提供的 `admin/admin` + `Personal` bootstrap contract。
...
- 每 User 一个不可删除、可改名、可加人的 Personal Team。
- Team switcher、创建普通 Team、重命名 Team、归档删除普通 Team；唯一 Team 与 Personal Team 不可删除。
- Settings > Team Members：按 username 添加、移除/停用成员并设置 Owner/Admin/Member。
- Team-level permission enforcement、最后 Owner 与 personal owner 保护。
...
- 每个 User 恰好拥有一个 Personal Team；Personal Team 可改名、可加人，但不可删除或转移 personal ownership。
- 初始化后允许公开本地注册；新 User 注册时原子获得自己的 Personal Team。
...
- **SC-002**：新 User 注册后，User、Personal Team、Owner membership 与 session 要么全部成功，要么全部不存在；每个 active User 的 Personal Team 数始终为 1。
...
- **SC-005**：Personal Team 可成功重命名和添加成员，但删除、转移 personal ownership、移除或降级 personal owner 的成功次数均为 0。
- **SC-006**：普通 Team 在 Owner 仍有其他 active Team 时可归档删除；Personal Team或当前唯一 active Team 的删除成功次数为 0。
- **SC-007**：Owner 可在一个 Team Members 页面完成添加成员、移除成员和设置 Owner/Admin/Member；重复添加、未知 username、last-owner 与 personal-owner 冲突均返回稳定结果。
...
- **SC-011**：SQLite、PostgreSQL 与 Supabase-backed PostgreSQL 运行相同 identity/Personal Team/RBAC contract suite，核心场景通过率均为 100%，schema parity 无未记录差异。
