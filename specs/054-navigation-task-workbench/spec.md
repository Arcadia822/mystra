...
**Feature Branch**: `N/A（与其他 spec 并行规划，不创建独立 spec 分支）`
...
**Independent Test**: 登录后访问产品根入口，验证首先显示 053 Overview；分别在 sidebar 展开、桌面收起和窄屏隐藏三种状态下，用一次操作打开 New Task modal 与 Search dialog，且当前页面不会先发生导航。

**Acceptance Scenarios**:

1. **Given** 操作者已登录，**When** 打开产品根入口，**Then** 系统显示 053 Overview，而不是 New Task 页面。
...
**Independent Test**: 准备包含全部五种 `productionStatus`、有无 Project、不同 Labels、以及有无 Issue reference 的 Task 数据；从主菜单进入 Tasks，验证默认无表头的堆叠列表显示六个默认字段，通过 Display 切换 Table/Kanban 与附加字段，执行搜索、筛选、刷新和打开详情，并验证切换布局不会丢失当前查看条件。
...
4. **Given** 一个 Task 没有 Project 或 Issue reference，**When** 它出现在任一布局，**Then** 系统使用明确的空上下文表达，不隐藏该 Task，也不伪造外部数据。
...
8. **Given** Kanban card 的 Project、Issue 与 Labels 总宽度超过卡片可用空间，**When** 组件完成布局或卡片宽度发生变化，**Then** 只显示能够完整容纳的前置 Labels，并用共享 `+N` overflow control 代表其余项目；激活该 control 后，标准全局浮动 popup 展示被折叠的完整 Label 列表，卡片内不得出现重叠、半截 Label 或换行增高。
8. **Given** 操作者同时显示 Updated At 与 Created At，**When** 比较两个时间字段，**Then** 两者使用同一个共享 time renderer、locale 与 format options，不出现一个字段为相对时间而另一个字段为绝对日期的混合文案。
...
6. **Given** New Task modal 已打开，**When** 操作者选择 Project，**Then** 标准 Project dropdown 展示 No project 与可用 Project（包含 repository 描述），支持指针和键盘选择，并在 footer trigger 中回显选择结果；提交 action 的可见文案为 `Create`。
...
**Independent Test**: 从 Table row、Kanban card 与 Active Tasks 分别打开同一个 fixture，验证 route 与 Task ID 一致；在 1440px、1024px 与 768px 宽度下验证 shell 主内容与全局 Right Panel，在 320px 下验证全局窄屏顺序；确认 header breadcrumb、New Session、New Task/Search、Project/Issue Labels 与 Sessions 均使用共享组件并保持键盘可达，且 Main 在 Sessions 之前没有 page-local identity、Production、Harness 或 Workspace 内容。
...
2. **Given** Task detail 已加载，**When** 操作者扫描首屏，**Then** shell header 负责 Task name，Main 直接从共享 Sessions stacked list 开始；Main 不重复 title、description、`productionStatus`、Production、Harness 或 Workspace，Project、Issue、Labels、Task ID、时间与 Status history 仍位于全局 Right Panel。
...
5. **Given** Task 有 Project、Issue 与 Labels，**When** 属性侧栏呈现这些值，**Then** Project、Issue 与每个 Label 均复用共享 `UiLabel` 及 provider icon；日期语义使用 `<time>`，但继承属性值的标准 typography。

---

### User Story 6 - 从 Task 手动发起新的 Session (Priority: P1)

作为正在监督 Task 的 Mystra 操作者，我希望从 Task detail 的主 Header 打开一个精简的 Create Session Modal，只填写本次 Session 的提示词并选择 Provider，以便在不接触系统解析的 Runtime、Agent Context、Task 生产状态或 Harness 归属的前提下，为同一个 ready Workspace 发起新的独立执行会话。
...
**Independent Test**: 打开一个 Workspace=`ready` 且 Runtime 至少有一个 available Provider 的 Task detail，使用 Header 右侧 New Session 打开 Modal，在 body 的唯一 Prompt input 填写提示词、在 footer 左侧选择 Provider，并用 footer 右侧 Create 提交；验证生产实现只把 Provider 与该提示词对应的 canonical `manualContext.text` 发送给当前 Task 的 Session launch API，由服务端从 ready Workspace 解析 Runtime，并省略/null `agentId`。成功后进入返回的 Session detail，Task `productionStatus` 与 Harness `sessionId` 均不被修改。再分别验证 Close、Workspace 未 ready、无可用 Provider 与 API 失败。
...
2. **Given** 操作者激活 New Session，**When** Modal 打开，**Then** header 标题逐字为 `Create Session` 且只包含统一 Close control；body 只包含一个 aria-label=`Prompt` 的提示词输入；footer 左侧是只列出该 Runtime available capabilities 的必选 Provider dropdown，右侧是文案逐字为 `Create` 的 primary action。Runtime 与 Agent Context 的可见文案、只读行、dropdown 或 placeholder 数量均为 0。
3. **Given** 操作者完成合法输入，**When** 提交 Create，**Then** 系统将 Prompt 展示值映射到 canonical `manualContext.text`，省略/null `agentId`，并由服务端从 ready Workspace 解析 Runtime 后为当前 Task 创建新的 Task-bound Session；UI 不新增 `prompt` domain field。成功后进入服务端返回的 Session detail；Task `productionStatus` 不改变，既有 Harness 的 `sessionId` 不增加或替换。
4. **Given** Workspace 不是 `ready` 或当前 Runtime 没有 available Provider，**When** 操作者尝试发起 Session，**Then** launch 明确不可用并说明原因，不发送请求，也不伪造成功。
5. **Given** Modal 已打开，**When** 操作者按 Escape、点击 header Close 或点击 backdrop，**Then** Modal 关闭且焦点回到 New Session action，Task route 与已填写持久状态均不改变；footer 不渲染重复的 Cancel action。
6. **Given** launch API 返回错误，**When** Modal 呈现失败，**Then** Modal 保持打开、保留当前输入并显示可理解错误；Sessions 列表、Task 状态与 Harness 均不被本地乐观伪造。
...
1. Main 不再拥有 page-local identity、Production、Harness 或 Workspace composition。
...
| --- | --- | --- | --- | --- |
| Production / Task state | `Task 1`；`productionStatus` 是 Task business state | `GET /api/tasks/:id/production` 的 `task`；基础身份可来自 `GET /api/tasks/:id` | 当前 Main 不渲染；保留为 ownership 核对依据 | 在 Sessions row 推断、同步或复制 Task 状态；Task `failed/error` |
| Production / Harness attempt | `Task 1 -- 0..1 Harness`；当前 v1 `Harness.taskId` unique；`Harness 1 -- 0..1 Session` | production projection 的 `harness`、`latestSession`、`promptEvidence` | 当前 Main 不渲染；不把 Harness facts 塞进 Session row | `currentAttempt`、attempt sequence、Harness status、把 latest Session 当作 current Session |
| Workspace | `Task 1 -- 0..1 TaskWorkspace`；Workspace `1 -- 0..N WorkspacePreparationAttempt`，attempt 不是顶级业务对象；所有 Task-bound Sessions 复用同一 ready Workspace attachment | `GET /api/tasks/:id/workspace` 的 `TaskWorkspaceView` | 当前 Main 不渲染；不把 Workspace facts 塞进 Session row | host absolute path、opaque `workspaceRef`、credential、虚构 repository full name、把 Workspace state 当 Session state |
| Sessions | `Task 1 -- 0..N Session`；每个 Session 独立保存 Runtime、Provider、optional Agent Context reference 与 execution state；Harness 最多只引用其中 `0..1` | `GET /api/tasks/:id/sessions?limit=50&cursor=...` 原样返回 `Session[]`；Runtime 显示名可由既有资源查询解析，缺失时回退稳定 ID | helper count；左 `state` icon label（等宽）、`providerKey` provider icon、完整 `Session.id` name；右 Runtime `UiLabel`（默认 hidden）、`updatedAt`；row navigation 到 Session detail | 截短 Session ID、`SessionSummary`/`TaskSession` DTO、mock `title`、objective label、completedAt、terminal summary、Workspace path、根据数组位置虚构 current/latest badge |
...
- **Production / Task、Harness、Workspace**：合同仍按上述实体独立存在，但当前 Main
...
- **独立失败**：Session failed 不得自动改变或暗示 Task、Harness、Workspace 状态；
...
| --- | --- | --- | --- |
| Start production | Task=`pending`，Project/Runtime/Provider 前置条件满足 | Human；canonical Task production command | 原子 `pending -> in_progress` 并创建唯一 Harness；提交后请求 Workspace preparation，ready 后幂等创建 Harness 的 `0..1` Session |
...
| Setup / Retry Workspace | absent+eligible 或 `failed`；`queued/preparing/ready/unavailable` 不提供相同动作 | Human Workspace action；Start 后也可由 production continuation 请求 setup | 只改变 Workspace/attempt facts；不改变 Task business state；不换 Runtime |
| New Session | Workspace=`ready` 且选定该 Runtime available Provider；Runtime 由服务端解析，Agent Context 在本 UI 中省略/null | Human Task Session action | Prompt copy 映射到 canonical `manualContext.text`，创建新的 Task-bound Session；不改变 Task `productionStatus`，也不增加 Harness 的 Session 数量 |
...
- 删除 “current execution attempt” 与 “current Session”；当前合同只有 singular
  `0..1 Harness`、`0..N Sessions` 和可选 `harness.sessionId`，没有 current 字段。
- 删除 “Agent is working in the current Session”；Task=`in_progress` 不能证明任一
  Session=`running`。
- 删除 `attempt 01`；Harness 没有 sequence 或展示编号，必须使用 Harness UUID。
...
- 主区域 fixture 的 Task/Harness/Workspace/Runtime/Session identity 必须使用 UUID；
  `MYS-118` 只能作为 Issue identifier。Right Panel 现有 route fixture 的 ID 表达不在
  本独立任务修改范围，必须作为原 task 的待办而不是在主区域复制。
- 删除 Main 中 Sessions 之前的 page-local Task identity、Production/Harness 卡和
...
- 053 Overview 尚未可用时，054 不应复制或临时伪造 Overview；根入口切换必须等待 053 的可用页面合同。
...
- Project 或 Issue 当前不可用时，Task 仍显示其 Mystra-owned title、status 与已保存引用边界，不把外部读取失败解释为 Task 不存在。
- 一个 Task 没有 Labels、拥有多个 Labels 或 Label 文案过长时，堆叠行必须保持 Name 可读，并以明确空值、截断或受控折叠表达 Labels，不把任意外部 metadata 当作 Task Label。
...
- **FR-001**: 已登录产品根入口 MUST 显示 feature 053 提供的 Overview；054 MUST 将 053 作为前置依赖，不复制其内容、查询或空态。
...
- **FR-014**: stacked Table 每行默认 MUST 只显示六种字段，并复用 CastrelTable 的弹性行模型：左侧固定区为 `productionStatus` icon，grow 区为 Name，中间 spacer 吸收剩余宽度，右侧堆叠 Project、Issue、Labels/Metadata、Created At。Project、Issue 与 Labels/Metadata 保持自然宽度；只有符合 `equalWidth` 边缘连续性合同的字段可以使用跨行统一列宽，不得为其他右侧值设置固定宽度来伪装成无表头 column grid。所有相邻 list field 使用同一个 column-gap token；一个 Metadata group 内多个 Label 的间距 MUST 与 Metadata group、Project、Issue 和日期之间的间距相同。点击任意 Task 行 MUST 打开 Task detail。
...
- **FR-020**: Project、Issue 与每个 Labels/Metadata value 或 key-value pair MUST 复用同一个 Mystra Label component。Label 的 key、value 与 `+N` overflow control MUST 使用标准 12px content text，集成 icon MUST 使用 16×16px 标准 icon slot；consumer MUST NOT 单独缩小 key 或 overflow text。Project Label MUST 在文字前显示其 Project repository Integration provider icon；Issue Label MUST 根据其 exact Issue reference 显示 GitHub 或 Linear provider icon。054 当前只要求 GitHub 与 Linear 两种 Issue icon；无 Project/Issue 时不得伪造 provider icon。
...
- **FR-025**: Task MUST 新增 Mystra-owned `labels` 字段，作为零个或多个受限标签值或 key-value pair 的一等属性供持久化、读取与列表呈现；MUST NOT 以任意 JSON `metadata` blob 代替 Labels，也不得从 Issue/Project 外部 metadata 临时推导。长度、数量、规范化、写入入口与重复规则 MUST 在技术计划中冻结并由 shared schema 与持久化合同共同执行；New Task modal 在 054 中仍只暴露 Project 配置。
- **FR-026**: Kanban MUST 按五个 `productionStatus` 分栏，并在卡片中至少展示 Status、Name、Project、Labels 与 Created At。每张 compact workbench card MUST 使用四边精确 8px 的主题 spacing inset，不得误用 12px generic content inset。每张卡片的 Name 区域 MUST 固定预留两行高度；不足两行时保留该高度，超过两行时在第二行末截断，使相同字段配置的卡片保持一致高度。卡片的 Project、Issue 与 Labels MUST 作为同一个有序 Label 集合使用共享 overflow primitive：组件 MUST 依据容器实际宽度和每个 Label 的自然宽度显示最长的完整前缀，并为未显示项保留一个同系列 `+N` control；激活后 MUST 通过标准全局 portal popover 列出全部被折叠 Labels，并支持 outside click、Escape、focus return、可访问名称与容器 resize 后重新测量。MUST NOT 以固定数量、重叠、部分裁切、换行增高或 card-local popup 代替该行为。
- **FR-027**: Kanban 第一版 MUST 是只读布局；MUST NOT 通过拖拽、列内创建或卡片快捷编辑改变 `productionStatus` 或其他 Task 字段。
- **FR-028**: Table 与 Kanban MUST 允许打开同一个 Task detail，并对空状态、加载、错误、超长标题、空 Labels 和缺失外部上下文提供等价且可理解的表达。
...
- **FR-039**: 054 MUST 直接采用五态 Task `productionStatus` 合同：`pending`（未执行）、`in_progress`（执行中）、`blocked`（待接手）、`done`（已完成）、`canceled`（已取消）。`waiting_for_review` MUST 从 Task 状态枚举、迁移合同和所有 Task UI 中移除，不得保留 alias、双读或展示层映射；原有 review handoff 语义并入 `blocked`。`blocked` MUST 表示“执行权需要 handoff 给 Human”，而不是 error 或单一阻塞原因，并继续要求非空 note。Review、授权、等待回答、等待信息等细分原因属于后续结构化 handoff reason，不得在 054 中重新扩展顶层状态。
- **FR-040**: 五态迁移 MUST 延续 actor ownership：Assign/Start 只执行 `pending -> in_progress`；Agent 可执行 `in_progress -> blocked` 与 `blocked -> in_progress`；Human 可执行 `blocked -> in_progress|done`，并可将任意非终态 `pending|in_progress|blocked` 置为 `canceled`；`done` 与 `canceled` 为终态。Task MUST NOT 提供 `error` 或 `failed` 状态，Session/Harness/attempt 的执行失败事实不得自动改变 Task `productionStatus`。
...
- **FR-044**: Task detail 首屏 MUST 以 `productionStatus` icon、24px title 与 12px description 形成主要身份区，并提供不长期暴露 form chrome 的 title/description 编辑状态。Task name MUST 作为 shell 主标题栏 breadcrumb 的当前节点；Task ID 只位于 Properties，不得替代 name 或与 title 争夺主标题层级。breadcrumb 层级之间 MUST 使用共享 16px right-chevron arrow，不得使用 page-local slash 或字符分隔实现。
...
- **FR-049**: Task detail Main MUST 直接从 Sessions shared stacked list 开始；Sessions 之前和之外不得渲染 page-local identity、title/description editor、Production、Harness、Workspace、概览卡或 action header。New Session 只属于 shell Main Header 右侧，不形成 Main content action header。Production/Harness/Workspace 数据合同仍保留为 source-of-truth 约束，但本 composition 不显示、复制或迁移它们。054 MUST NOT 修改 breadcrumb、Right Panel 收展、Right Panel Properties、Status history 或全局 typography；FR-048 的 heading token 仅约束实际存在的 shared headings，不要求为 Sessions 新增 heading。
- **FR-050**: Production Task state MUST 继续被定义为 Task business state，Harness facts MUST 继续只来自 `harness | null`、`latestSession | null`、prompt evidence 与 Runtime display resolution；当前 Main 不渲染这些 facts，也不得把它们压入 Session row。任何实现 MUST NOT 引入、模拟或命名 `currentAttempt`/`currentSession` 字段，也不得从 `productionStatus` 推断 Session state。
...
- **FR-053**: Sessions state copy MUST 覆盖 049 的九态，明确 `ready` 可继续而非 completed，`closed|failed` 才是终态。列表必须区分 loading、request error、empty、rows 与 next-cursor loading；Session/Workspace/Harness failure 与 Task status 必须独立并列。
...
- **FR-055**: Main-area prototype mock MUST 使用与 `Task`、`Harness`、`TaskWorkspaceView`、`RuntimeView`、`Session` 同名字段、canonical enum literal、UUID identity、ISO datetime、合法 Git branch/ref 与 40/64 hex commit。展示名、短 commit、状态文案和相对时间必须是 renderer 派生值，不得回写成 mock domain fields。
...
- **FR-060**: Production launch MUST 使用当前 Task ID 调用 canonical Task Session API，生成/提交 UUID `sessionId`、必选 `providerKey`、省略/null `agentId` 与可选 trimmed `manualContext.text`。UI 的 `Prompt` 只是 `manualContext.text` 的 presentation copy，不得新增 `prompt` request/domain field，也不得声称改写服务端固定 `firstUserMessage`。`runtimeId` MUST NOT 由表单提交；服务端继续从当前 ready Workspace 解析。成功 MUST 进入返回的 Session detail；失败 MUST 保留 Modal 输入并显示错误。Create MUST NOT 修改 Task `productionStatus`、Workspace identity/state 或既有 Harness `sessionId`。Prototype MUST 停在 dispatch 边界，不得追加 mock Session 或伪造成功。
...
- **Active Task Shortcut**: Team 范围 Task 的快速导航投影，只包含三种非终态 `productionStatus`，并按可选 Project context 分组；它不是完整 Tasks 查询或新的持久化对象。
...
- **Task Label**: Mystra-owned、Team-authorized 的 Task 一等属性；零个或多个受限标签值随 Task 持久化和读取，不是任意 metadata 容器，也不是外部 Issue label 的隐式副本。
...
- Feature 053 必须先提供可用的 Overview 页面；054 只拥有根入口与导航重排。
- “活跃 Task”固定表示 `pending`、`in_progress`、`blocked`；它不引入新的状态或派生持久化字段。
- 054 以 pre-0.1 直接替换方式收敛 051 的六态 Task 合同：删除 `waiting_for_review`，将其 handoff 语义并入 `blocked`，不创建兼容 alias、迁移 shim 或双写路径。技术计划必须同步 shared Task/Harness schema、迁移白名单、TaskStatusService、`mystra-agent`、标准执行 prompt、API/UI、RDB fixtures 与 tests。
...
- 本功能不新增业务实体或 view preference 持久化模型，但会扩展现有 Task 合同与持久化记录以承载一等 `labels`；054 不引入通用 `metadata` 字段。
- Owner 正在并行规划多个 specs；054 的规划产物保留在当前工作分支，不以独立 Git branch 作为需求成熟度或所有权边界。

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: 100% 的已登录根入口验收用例首先显示 053 Overview，且不再显示或跳转 New Task page。
...
- **SC-004**: 对覆盖五种 `productionStatus` 和多个 Project context 的验收数据，sidebar 快速区对 `pending`、`in_progress`、`blocked` 的展示准确率为 100%，Project 分组准确率为 100%，对 `done`/`canceled` 的误展示为 0。
...
- **SC-009**: Mystra 基础表格验收 fixture 证明 stacked/column mode、搜索、筛选、排序、字段显隐、异步追加、空/加载/错误状态与可选行选择均可由页面独立配置；Tasks consumer 默认只出现五个字段、零个 column-title row、零个 footer 和零个分页器。
- **SC-010**: 对 Refresh 的静态、加载和完成状态截图比较中，三个 toolbar actions 均为 Mystra ghost icon action；Refresh 在加载前后使用同一个 glyph，加载时持续旋转且完成后停止。
- **SC-011**: Task API/持久化合同 fixture 对空 Labels、多 Labels、重复输入、超长输入与非法值给出稳定结果，且序列化输出不出现通用 `metadata` 属性。
...
- **SC-019**: 使用 0、1、3、10 个不同宽度 Labels，并在至少三种 Kanban card 宽度下验收：可见 Labels 均完整且互不重叠，`+N` 数值与隐藏项数量 100% 一致；最窄情形只显示 `+N`，popup 完整列出所有隐藏项；扩大卡片后可见前缀自动增长且 `+N` 相应减少或消失。
...
- **SC-023**: Task detail 静态依赖审计确认 shell、`UiBreadcrumb`、surface sections、actions、status icons、Project/Issue/Labels 均来自 `@mystra/ui`，feature component 中手写 `<nav>`、breadcrumb separator、局部 `taskDetailAside`、SVG、复制 standard component DOM anatomy 或 page-local raw color 的数量均为 0。
...
- **SC-025**: Task detail Main 在 1440px、768px 与 320px 的第一个且唯一内容集合均为 Sessions shared stacked list；Sessions 之前的 page-local identity、Production、Harness、Workspace、概览卡、caption、column header 与 action header 数量均为 0。列表只在自身 viewport 内横向溢出，page-level horizontal overflow 为 0。
- **SC-026**: fixture 静态 schema 审计中 Task/Harness/Workspace/Runtime/Session ID 的 UUID 合格率为 100%，ISO datetime、Git ref/branch 与 40/64 hex commit 合格率为 100%；主区域不存在 `currentAttempt`、`currentSession`、`attempt 01`、raw `Completed`、host absolute path、workspaceRef 或虚构 repository full name。
...
- **SC-028**: 用 Task=`in_progress`+Session=`failed`、Task=`blocked`+Session=`ready`、Workspace=`failed`+Task=`in_progress` 三组 fixture 验收 presentation mapping 时，Sessions 只显示自身 state，不从未渲染的 Task/Workspace/Harness facts 自动映射 badge 或 copy；Task UI 中 `failed/error` production status 出现次数为 0。
...
- **SC-033**: production component/browser tests 覆盖 ready+available 成功、Workspace 非 ready、无 available Provider 与 API error 四条路径；成功请求 100% 通过 `taskSessionLaunchInputSchema`，成功导航使用返回 Session ID，所有路径对 Task `productionStatus` 与 Harness `sessionId` 的非预期写入次数为 0。
...
- Current lifecycle evidence: `packages/shared/src/task.ts` 与 `packages/shared/src/harness.ts` 仍包含 `waiting_for_review`；`apps/control-plane/src/lib/tasks/task-status-service.ts`、`packages/agent-cli`、标准执行 prompt、API/UI 与 tests 均消费该旧合同。054 规划必须一次性替换这些调用面；原型不是兼容层。
- Task detail main-area source evidence: `packages/shared/src/{task,harness,session,task-workspace}.ts`、SQLite/PostgreSQL Prisma schemas、`RdbProvider`、`GET /api/tasks/:id/production`、`GET /api/tasks/:id/sessions`、`GET /api/tasks/:id/workspace` 与 production Task detail panels。048/049/050/051/052 分别拥有 Workspace、Session、Task Session UX、Harness/Task status 与 optional Agent Context 边界；prototype mock 与旧 UI 均不拥有领域合同。
