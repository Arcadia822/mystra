...
5. **Given** New Task modal 已打开，**When** 操作者查看其结构，**Then** header 使用共享标题与唯一 Close control，title input 和 description 位于 body，可选 Project 配置与 Create action 位于 footer；Header/Body/Footer 由公共 Section slots 提供，不复制 Modal chrome。
...
- **FR-030**: New Task modal MUST 使用标准 `UiDialogSurface`、`UiSurfaceHeader`、`UiSurfaceBody` 与 `UiSurfaceFooter`，不得使用 feature-local Header/Body/Footer DOM 或 compact-row `layout="rows"` 补偿。Header 与 Footer MUST 各为 44px，并由公共 slot 提供 8px 横向 padding 和 divider；Header 使用 `UiSurfaceTitle` 显示 `Create Task`/`创建 Task`，唯一 action 是标准 `UiDialogCloseButton`。Body MUST 只从公共 slot 获得 8px 横向 padding，并由 `taskComposerBody` 自行提供 8px 上下 padding；`Task name` 的共享 `UiInput` 与无边框、透明、不可 resize 的 description `UiTextarea` 都位于 Body。当前唯一 Task 配置 Project MUST 位于 Footer 并使用标准 `UiDropdown` ghost variant；trigger 宽度 MUST 由内容/owning row 自然决定，menu MUST portal 到全局 popup layer并保留 No project、可用 Project、repository 描述、选中回显、outside/Escape/Arrow/Home/End/Tab 行为。Project trigger 与 solid `Create` action MUST 使用共享 28px `inline` size；Footer 不渲染 Cancel。
...
- **FR-059**: New Session Modal MUST 复用 shared `UiDialogSurface` compact-row composition、`UiSurfaceHeader/Body/Footer`、`UiDropdown`、`UiTextarea`、`UiButton` 与统一 close control。Surface 只拥有一次 8px inset 与 8px section gap；header/footer MUST 是无独立 padding/divider 的 44px rows。Header 标题 MUST 逐字为 `Create Session`，且 Close glyph 是唯一 header action。Body MUST 只有一个 aria-label=`Prompt`、最多 `SESSION_TEXT_MAX_LENGTH` 的文字输入；该 input MUST 无边框、透明背景、不可 resize，并保留 placeholder `Session-only context, constraints, or a specific focus`。Footer MUST 左对齐必选 available Provider dropdown、右对齐文案逐字为 `Create` 的 solid inline action；两者为 28px controls。Runtime row、Runtime selector、Agent Context、intro/notice、Cancel 与 `Launch Session` 的出现次数 MUST 为 0。Modal MUST 支持 backdrop、Escape 与 Close，关闭后焦点返回 trigger；窄屏不得产生 page-level overflow。
- **FR-061**: Search Modal MUST 使用共享 `UiDialogSurface`、44px `UiSurfaceHeader`、`UiDialogCloseButton` 与 `UiSurfaceBody`。Header 内的搜索 icon、共享 `UiInput` 与 Close control 不得使用私有 48px header、15px input 或 copied icon button。Modal workspace MUST 直接作为 `UiSurfaceBody`，公共 Body 只提供左右 8px padding且上下为 0；业务 split-pane 不得添加上下 padding，使 list/preview 之间的竖向 divider 从 Body 顶部连续到达底部。Actions、Tasks results 与选中 Task preview MUST 复用 `UiSurface`、`UiSurfaceHeader`、`UiSurfaceTitle`、`UiSurfaceBody` anatomy；业务内容可在自己的 Body class 中声明上下 padding。
...
- **FR-061**: 每个 Task-bound Session MUST 在 dispatch 时获得自己独立、短期、可撤销的 execution code，并由 Runner 注入 Runtime 所提供 `mystra-agent` 的绝对路径 `MYSTRA_AGENT_PATH`；Session 在执行前 MUST 通过 `"$MYSTRA_AGENT_PATH" context get` 解析该 Task 唯一 TaskExecutionContext 下的权威 Task、Project、Issue reference、共享 Workspace 与 capability facts。Runtime CLI 的实时响应及其接受的命令 MUST 覆盖 Workspace 内冲突的源码、文档或生成 CLI；即使 Task 的目标仓库是 Mystra 本身，Agent 也 MUST NOT 构建或调用 Workspace 副本中的 `mystra-agent`。后续 Session MUST NOT 复用首个 Session 的 code，也不得覆盖 `TaskExecutionContext.sessionId` 保存的首个 Autopilot Session 关联。所有 Task-bound Session 均接收同一份 content-addressed Standard Execution Prompt。
...
- **SC-015**: New Task modal 的静态与键盘验收中，Header/Footer computed height 均为 44px、公共横向 padding 均为 8px，Body computed padding 为 `8px 8px`（横向由公共 slot、纵向由业务 class 各自拥有）；Project trigger 与 Create action computed height 均为 28px。Header title 为 `Create Task`/`创建 Task`，title input 与 description 位于 Body，description resize handle 为 0，Project dropdown 可选择 3 个样例值且提交文案精确为 `Create`。
...
- **SC-032**: Task detail 在 1440px、768px 与 320px 下均只显示一个 Header New Session action；Right Panel 收起时 recovery control 排在其后。Modal header/footer computed height 均为 44px，header/footer 在 compact-row composition 中自有 padding 与 divider 均为 0；header 标题为 `Create Session`，body 可交互元素只有 1 个 Prompt input，其 computed border width 为 0px、background 为 transparent、resize 为 none 且 placeholder 逐字匹配 FR-059；footer 顺序为左 28px Provider dropdown、右 28px Create。Runtime/Agent Context/Cancel/Launch Session/intro/notice 的 DOM 数量均为 0；Escape、backdrop 与 Close 均能退出并将焦点返回 trigger，page-level horizontal overflow 为 0。
- **SC-035**: Search Modal computed Header height 为 44px、workspace Body padding 为 `0px 8px`；list pane 的 right divider 顶部与 Body 顶部差值、底部与 Body 底部差值均为 0px。Close 使用共享 `UiDialogCloseButton`，Actions/Tasks/preview 三处 Section 标题均为 `UiSurfaceTitle` 的 12px/500 typography；Modal DOM 中 `compactIconButton`、15px search input 与 feature-local section header anatomy 的数量均为 0。
