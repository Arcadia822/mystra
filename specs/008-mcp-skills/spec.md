...
**Implementation Note**: The current implementation ships repo-local companion skills under `.agents/skills/` for user-journey submission, implementation-request submission, and status lookup, plus a feature quickstart describing discovery and installation in this repository's local skill-pack model.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Agent Submits a User Journey Through a Skill (Priority: P1)

As an AI agent working on a product, I want to invoke a companion skill that packages a user journey (actor, goal, acceptance criteria) into a Mystra task, so that Mystra can implement the journey on a target project without me manually constructing the MCP task payload.
...
**Independent Test**: Invoke the "submit user journey" skill with a project, actor, goal, and acceptance criteria; verify a Mystra task is created with the journey encoded in the prompt.

**Acceptance Scenarios**:

1. **Given** a companion skill for user journey submission is installed, **When** an agent invokes it with project, actor, goal, and acceptance criteria, **Then** a Mystra task is created with a structured prompt that encodes the journey.
2. **Given** the skill accepts optional context (existing spec references, design decisions), **When** context is provided, **Then** it is included in the task prompt or as context bundle references.
...
**Independent Test**: Invoke the "submit implementation" skill with a project, spec reference, and task scope; verify a Mystra task is created with the implementation context.

**Acceptance Scenarios**:

1. **Given** a companion skill for implementation submission is installed, **When** an agent invokes it with project, spec reference, and task scope, **Then** a Mystra task is created with a prompt that includes the spec and task context.
2. **Given** the skill accepts a workflow blueprint name, **When** specified, **Then** the task is associated with that blueprint.

---

### User Story 3 - Agent Checks Task Status Through a Skill (Priority: P2)
...
**Why this priority**: Status checking is needed but less critical than submission. Agents can fall back to `mystra_get_task` directly.

**Independent Test**: Invoke the status skill with a task id; verify it returns human-readable status and result summary.

**Acceptance Scenarios**:

1. **Given** a task was previously submitted, **When** the agent invokes the status skill, **Then** it returns status, progress summary, and result if available.
2. **Given** the task has completed with an MR/PR, **When** the skill returns the result, **Then** it includes the MR/PR URL for review.
...
- What if multiple agents submit tasks simultaneously? Skills must not serialize submissions; each invocation is independent.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: A "submit user journey" skill MUST accept project, actor, goal, acceptance criteria, and optional context, and create a Mystra task.
- **FR-002**: A "submit implementation" skill MUST accept project, spec reference, task scope, and optional workflow name, and create a Mystra task.
- **FR-003**: A status skill MUST accept a task id and return status, progress, and result.
