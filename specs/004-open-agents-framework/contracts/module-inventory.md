...
| repository provider | fork | `apps/web/app/workflows/chat-post-finish.ts` | `packages/shared/src/repository.ts` | `framework-alignment.md: repository integration` | `fork-rules.md: Rule 1`, `fork-rules.md: Rule 2` | `010-repo-provider-contracts` | MVP review delivery covers GitLab and GitHub. The shared request/result contract now lives in `packages/shared/src/repository.ts`, while runner/provider implementations remain follow-on work. |
| sandbox provider | fork | `apps/web/app/workflows/sandbox-lifecycle.ts`, `apps/web/SANDBOX-LIFECYCLE.md` | `packages/shared/src/sandbox.ts` | `framework-alignment.md: sandbox execution` | `fork-rules.md: Rule 1`, `fork-rules.md: Rule 2` | `011-docker-sandbox-provider` | Mystra now anchors the sandbox launch/session/outcome contract in `packages/shared/src/sandbox.ts`, while the Docker runner path remains the first implementation target. |
